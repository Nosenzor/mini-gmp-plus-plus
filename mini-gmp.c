...
mpz_swap (mpz_t u, mpz_t v)
{
  ...
  /* u needs to point to u local buffer if v was using local array ... */
  if(u->_mp_d == v->_mp_buff) u->_mp_d = u->_mp_buff;
  /* ... and same thing for v ! */
  if(v->_mp_d == u->_mp_buff) v->_mp_d = v->_mp_buff;
}
#else
void
mpz_swap (mpz_t u, mpz_t v)
